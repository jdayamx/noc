...
def _format_iptables_save_to_commands(table_name, output):
    lines = [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "",
        f"# Generated by NOC at {datetime.now().isoformat(timespec='seconds')}",
    ...
    for raw_line in output.splitlines():
        ...
        if line.startswith("*"):
            lines.append(f"# Table {line[1:]}")
            continue

        if line.startswith(":"):
            chain_name, remainder = line[1:].split(" ", 1)
            policy = remainder.split(" ", 1)[0]
            if policy == "-":
                lines.append(f"iptables -t {table_name} -N {chain_name}")
            else:
                lines.append(f"iptables -t {table_name} -P {chain_name} {policy}")
            continue

        if line.startswith("-A "):
            lines.append(f"iptables -t {table_name} {line}")
            continue

    lines.append("")
    return "\n".join(lines)

def get_iptables_command_script():
    if platform.system() == "Windows":
        return ""
    ...
    for table_name in ("filter", "nat", "mangle", "raw", "security"):
        ...
        export_chunks.append(_format_iptables_save_to_commands(table_name, output))

    return "\n".join(export_chunks).strip() + ("\n" if export_chunks else "")
